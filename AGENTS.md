...
- XML parsing is done with wrappers in `src/xml.rs`.  These are wrappers for the `quick-xml` crate.  The preferred pattern for code that parses XML is to have a `Phase` enum that identifies where in the XML structure one is, and to match on a tuple of the `Phase` and the `name()` of the XML.  In general, it is not necessary for specific `Phase` entries to exist if the tag is a "leaf" tag and only attributes are used.  If significant enough, have a `State` struct with methods like `handle_start()` and `handle_end()`. Examples of this pattern are in `src/info/build.rs`, `src/status/parse.rs`, `src/history_xml/parse.rs` and `src/diagnostics/script.rs`.

- Miscellaneous
  - Custom serializers for preferences are in modules named `src/prefs/serde_*.rs` and are referenced with `#[serde(with = "serde_*")]`
  - In unit tests, it is preferred to assert in the form `assert_eq!(expected, actual)` in that order